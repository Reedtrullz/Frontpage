...
function SectionHeader({ title }: { title: string }) {
  return <h2 className="font-mono text-sm text-green-500 mb-4">{title}</h2>;
}
...
type AdminClientProps = {
  initialPersonal: PersonalData | null;
...
export function AdminClient({
  ...
  const [personal, setPersonal] = useState<PersonalData | null>(initialPersonal);
  ...
  if (!personal) {
    return <p className="text-sm text-red-400">{error || "Failed to load data"}</p>;
  }
